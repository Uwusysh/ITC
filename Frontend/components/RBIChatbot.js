  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setMessages(prev => {
        const newMessages = [...prev];
        if (newMessages[newMessages.length - 1]?.type === 'status') newMessages.pop();
        return [...newMessages, { text: "Sorry, there was an error processing your request.", sender: 'bot', type: 'error' }];
      });
    } finally {
      setIsLoading(false);
    }
...
<div className="chatbot-messages">
  {messages.length === 0 ? (
    <div className="welcome-message">
      <p>Welcome! Ask me anything about Bank Audit</p>
      <p>I'll provide answers from RBI documents, web sources, and Guidance Note on Audit of Banks 
        (2025 Edition). <br/>Created by <i>Radha singh, Shruti Tiwari and Manvi Pandey</i>.</p>
    </div>
  ) : (
    (() => {
